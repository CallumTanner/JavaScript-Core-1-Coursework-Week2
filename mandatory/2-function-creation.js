...
function buyTwoGetTheCheapestFree(price1, price2) {
  if (price1 <= 0 || price2 <= 0) {
    return "Invalid prices. Prices must be greater than zero.";
  }
...
function countReverse(number) {if (number <= 0 || !Number.isInteger(number)) {
  console.log("Invalid input. Number must be a positive integer.");
  return;
}
